...
import { Cloud, CloudOff, Wifi, WifiOff, Crown, ShieldCheck } from 'lucide-react';
...
const NexusStatus: React.FC = () => {
  ...
  useEffect(() => {
    // Suscribirse a cambios en el servicio cloud
    const unsubscribe = cloudService.subscribeToNexusStatus((online, currentPlan) => {
  ...
  const getPlanLabel = () => {
      switch (plan) {
          ...
          case 'pro': return 'PRO';
          default: return 'STARTER'; // Ocultar para free si prefieres minimalismo
      }
  };
  ...
  return (
    <div className={`flex items-center gap-2 px-2 py-1 rounded-md text-xs font-bold transition-all duration-500 ${statusColor}`}>
